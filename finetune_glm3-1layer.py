...
def main(
    ...
    def patch_glm_block(layer_instance, layer_index, global_model_config, device_to_use):
        if not hasattr(layer_instance, 'local_conv'):
            local_conv_instance = LocalConv1D( # 使用你提供的 LocalConv1D
                config=global_model_config, # 传递顶层模型配置
                kernel_size=3,
                device=device_to_use,
                dtype=None # LocalConv1D 内部会强制 FP32 权重
            )
            setattr(layer_instance, 'local_conv', local_conv_instance)
            setattr(layer_instance.local_conv, 'layer_number', layer_index)

            # Dropout rate 从全局配置获取
            dropout_rate = getattr(global_model_config, 'hidden_dropout', 0.0) # ChatGLM3 默认为 0.0
            ...
            # 这个 patched_forward 需要与 GLMBlock.forward 的签名和内部逻辑（特别是残差和 layernorm）对齐
            def patched_forward(
                    self,
                    hidden_states,
                    attention_mask,
                    ...
                    kv_cache=None,
                    use_cache=True,
                    # **kwargs # 移除 kwargs 如果原始 GLMBlock.forward 没有它
            ):
                ...
                # 调用 LocalConv1D
                local_conv_res = self.local_conv(layernorm_output) # 输入 layernorm_output
                local_conv_res_dropout = self.local_conv_dropout(local_conv_res)

                # 融合逻辑 (参考 GLMBlock.forward 源码)
                # ChatGLM3: apply_residual_connection_post_layernorm = False, hidden_dropout = 0.0
                ...
                # 原始: layernorm_input = F.dropout(attention_output, p=self.hidden_dropout, training=self.training) + residual1
                # 修改后:
                ...
                if not (residual1.dtype == attention_output_dropout.dtype == local_conv_res_dropout.dtype):
                    # print("Type mismatch before first sum...") # 调试打印
                    common_dtype_for_sum1 = attention_output_dropout.dtype # 或者 hidden_states.dtype
                    residual1 = residual1.to(common_dtype_for_sum1)
                    local_conv_res_dropout = local_conv_res_dropout.to(common_dtype_for_sum1)

                layernorm_input_for_mlp_ln = residual1 + attention_output_dropout + local_conv_res_dropout

                layernorm_output_for_mlp = self.post_attention_layernorm(layernorm_input_for_mlp_ln)
                mlp_res = self.mlp(layernorm_output_for_mlp)

                if apply_res_post_ln:
                    residual2 = layernorm_output_for_mlp
                else:
                    residual2 = layernorm_input_for_mlp_ln
                
                mlp_res_dropout = F.dropout(mlp_res, p=current_hidden_dropout, training=self.training)
                final_block_output = residual2 + mlp_res_dropout

                return final_block_output, kv_cache_updated # 与原始 GLMBlock.forward 返回值一致
    ...
    # --- 只对第一层应用 Patcher ---
    print("-" * 30)
    print("Applying layer modification to the first GLMBlock...")
    layers_added_count = 0
    
    if layers_to_process_list:
        first_layer_index = 0
        first_layer_instance = layers_to_process_list[first_layer_index]

        if isinstance(first_layer_instance, GLMBlock_cls):
            if patch_glm_block(first_layer_instance, first_layer_index, model_config, target_device):
                layers_added_count += 1
                print(f"  Successfully patched the first GLMBlock (index {first_layer_index}).")
                print_model_size(model) # 应该显示0个新增可训练参数，因为 LocalConv1D 被冻结
            else:
                print(f"  Skipping patch for the first GLMBlock (index {first_layer_index}), already patched or not applicable.")
        else:
            print(f"!!! ERROR: The first layer (index {first_layer_index}) is not an instance of {GLMBlock_cls.__name__}. Cannot patch.")
    else:
        print("!!! WARNING: No GLMBlock layers found to patch.")
