...
class Seq2SeqTrainer(_Seq2SeqTrainer):
    ...
    # ****************************************************************************************
    #     [overwrite] Trainer.compute_loss
    # ****************************************************************************************
    def compute_loss(self, model, inputs, return_outputs=False):
        loss, outputs = super().compute_loss(model, inputs, return_outputs=True)
        print(f"[debug-loss] {loss}, {len(inputs['input_ids'])}")
    ...
    def seed_worker(worker_id):
        """
        Function to seed workers, if needed for deterministic behavior.
        """
...
def process_batch(
    ...
    for tools, conv in zip(batched_tools, batched_conv):
        ...
        for message in conv:
            print(type(message), message)  # 打印消息类型和内容
